...
from WideAndDeep.NeuralNetUtil import network, network_preformance
np.random.seed(7)
set_random_seed(7)
...
class EntityEmbeddingNeuralNet(object):
    ...
    def data_write(self):
        np.save(os.path.join(self.__output_path, "train_init_score_2.npy"), self.__val_preds)
        np.save(os.path.join(self.__output_path, "test_init_score_2.npy"), self.__sub_preds)
